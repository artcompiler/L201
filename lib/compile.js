...
var translate = (function () {
      ...
      if (!(val instanceof Array) || !val.length) {
        ...
        if (_typeof(val[0]) === "object" && val[0] && val[0].key) {
          ...
          val.forEach(function (element, index, array) {
            ...
            if (_typeof(element[0]) === "object" && element[0] && element[0].key) {
              //one object (0 = goal, 1 = value)
              if (element[0].key === "goal" && element[1] && element[1].key && element[1].key === "value") {
                if (isNaN(element[0].val) || isNaN(element[1].val)) {
  ...
  function labels(node, options, resume) {
    ...
    visit(node.elts[1], options, function (err2, val2) {
      if (typeof val2 === "string" && val2.startsWith('key')) {
        var key = true;
  ...
  function background(node, options, resume) {
    visit(node.elts[0], options, function (err, val) {
      ...
      } else if ((typeof val === "undefined" ? "undefined" : _typeof(val)) === "object" && val && !isNaN(val.r)) {
          ret.bg = val;
        } else {
